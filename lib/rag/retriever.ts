...
const SYNONYMS: Record<string, string[]> = {
  ...
  ia: ["ai", "machine learning", "llm", "rag", "inteligencia artificial"],
  ai: ["ia", "machine learning", "llm", "rag"],
...
function scoreChunk(chunk: Chunk, queryTokens: string[]): number {
  ...
  const boost = SOURCE_BOOST[chunk.source] ?? 1
  return score * boost
}
...
export function retrieve(query: string, options: RetrieveOptions): RetrieveResult {
  const { locale, limit = 5, minScore = 2 } = options
